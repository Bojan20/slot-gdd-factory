...
(function () {
  ...
  function renderDetail(block) {
    ...
    /* Z.2 — Live iframe demo card.
     * Embeds a minimal slot HTML where THIS block is force-enabled, so
     * trigger presets fire into the demo's HookBus and the block runtime
     * actually executes in isolation. Demo files are generated by
     * tools/gen-block-demos.mjs (one HTML per block in blocks/demos/). */
    const demoSrc = './demos/' + (block.name || '').replace(/\.mjs$/, '') + '.html';
    const demoCard = el('section', { class: 'play-card', style: 'grid-column: 1 / -1;' }, [
      el('h3', {}, [
        'Live demo',
        el('small', { style: 'color: var(--p-fg-3, #8a96aa); font-weight: normal; margin-left: 8px;' },
          ' minimal slot with THIS block force-enabled'),
      ]),
      el('div', { class: 'play-demo-frame-wrap',
        style: 'position:relative;border:1px solid var(--p-fg-4,#2a3346);border-radius:8px;overflow:hidden;background:#0a1018;' }, [
        el('iframe', {
          id: 'demoFrame',
          src: demoSrc,
          title: 'Live demo of ' + block.name,
          loading: 'lazy',
          style: 'width:100%;height:480px;border:0;background:#0a1018;',
        }),
      ]),
      el('div', { class: 'play-button-row', style: 'display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;' }, [
        el('button', { class: 'play-btn', type: 'button', onclick: () => {
          const fr = $('#demoFrame'); if (fr) fr.src = fr.src;
        } }, 'Reload demo'),
        el('button', { class: 'play-btn', type: 'button', onclick: () => attachHookBus() },
          'Attach iframe HookBus'),
        el('a', { class: 'play-btn', href: demoSrc, target: '_blank', rel: 'noopener',
          style: 'text-decoration:none;display:inline-flex;align-items:center;' }, 'Open in new tab'),
      ]),
    ]);

    const grid = el('div', { class: 'play-detail-grid' }, [
      exportsCard, listensCard, emitsCard, filesCard, configCard, actionsCard, demoCard, liveCard,
    ]);
    ...
    /* Z.5 — best-effort attach + replay existing log rows. Z.2: also try to
     * attach to the demo iframe's HookBus when it finishes loading. */
    attachHookBus();
    const iframe = $('#demoFrame');
    if (iframe) {
      iframe.addEventListener('load', () => {
        // Slight delay so the slot's inline runtime has wired HookBus
        setTimeout(() => attachHookBus(), 200);
      });
    }
  ...
  function runPreset(preset, btnEl) {
    /* Z.2 — fire into demo iframe's HookBus first (where the block actually
     * lives), fall back to host window HookBus. */
    let bus = null;
    try {
      const fr = document.getElementById('demoFrame');
      if (fr && fr.contentWindow && fr.contentWindow.HookBus) bus = fr.contentWindow.HookBus;
    } catch (_) {}
    if (!bus && typeof window !== 'undefined') bus = window.HookBus;
  ...
  function attachHookBus() {
    /* Z.2 — prefer the demo iframe's HookBus (where the block actually
     * runs in isolation), fall back to host window HookBus, then no-op. */
    let bus = null;
    let busHost = 'iframe';
    try {
      const fr = document.getElementById('demoFrame');
      if (fr && fr.contentWindow && fr.contentWindow.HookBus) {
        bus = fr.contentWindow.HookBus;
      }
    } catch (_) { /* same-origin guard */ }
    if (!bus && typeof window !== 'undefined' && window.HookBus) {
      bus = window.HookBus;
      busHost = 'host-window';
    }
    if (!bus || typeof bus.on !== 'function') return false;
    if (bus.__playgroundAttached) { state.busAttached = true; state.busHost = busHost; return true; }
    bus.__playgroundAttached = true;
    state.busAttached = true;
    state.busHost = busHost;
    ...
    /* Z.2 — clear any empty-state hint so the operator sees they're live. */
