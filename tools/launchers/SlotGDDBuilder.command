...
# ── 1.5. Git auto-sync (Boki "updateuj .command da povuce uvek sve promene" 2026-06-21) ──
# Pull origin/main pre start-a tako da launcher uvek vrti najnoviji kod.
# Fail-safe: ako pull pukne (uncommitted local changes, network), warn-uj
# ali ne blokiraj launch — Boki može i bez pull-a da debug-uje lokalno.
if command -v git >/dev/null 2>&1; then
  if git -C "$PROJECT_DIR" rev-parse --git-dir >/dev/null 2>&1; then
    info "git auto-sync: fetching origin/main…"
    if git -C "$PROJECT_DIR" fetch --quiet origin main 2>>"$LOG_FILE"; then
      LOCAL=$(git -C "$PROJECT_DIR" rev-parse HEAD 2>/dev/null || echo "?")
      REMOTE=$(git -C "$PROJECT_DIR" rev-parse origin/main 2>/dev/null || echo "?")
      if [ "$LOCAL" = "$REMOTE" ]; then
        ok "repo up-to-date (HEAD = ${LOCAL:0:7})"
      else
        info "pulling ${LOCAL:0:7} → ${REMOTE:0:7}…"
        if git -C "$PROJECT_DIR" pull --ff-only --quiet origin main 2>>"$LOG_FILE"; then
          NEW=$(git -C "$PROJECT_DIR" rev-parse HEAD 2>/dev/null || echo "?")
          ok "pulled origin/main → HEAD ${NEW:0:7}"
        else
          warn "git pull failed (likely uncommitted local changes) — using current HEAD ${LOCAL:0:7}"
        fi
      fi
    else
      warn "git fetch failed (offline?) — using current HEAD"
    fi
  else
    warn "$PROJECT_DIR is not a git repo — skipping auto-sync"
  fi
else
  warn "git binary not on PATH — skipping auto-sync"
fi
...
# ── 5. AUTO-OPEN browser — Boki: "ne zelim da manuelno otvaam" (09.06.2026) ──
# Cache-bust query na svaki klik pa Chrome ne servira stale verziju.
BUST=$(date +%s)
...
T1=$(date +%s)
ELAPSED=$((T1 - T0))
osascript -e "display notification \"Slot GDD otvoren za ${ELAPSED}s\" with title \"Slot GDD Builder\" sound name \"Glass\"" 2>/dev/null || true
