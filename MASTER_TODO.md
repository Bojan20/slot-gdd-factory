...
> **🚨 W47.S1 OPEN (2026-06-15 17:30) — `Illegal return statement` regresija na WoO sample**. Live eyes-ultimate-qa otkrila 3 cascading fail-a na `sample/WRATH_OF_OLYMPUS_GAME_GDD` (10/13 PASS): ❌ `no page error` (`SyntaxError: Illegal return statement`) → ❌ `HookBus on window` (bootstrap crashed kao posledica) → ❌ `tap → preSpin` (events empty `[]` jer HookBus ne postoji). Diagnostic: single script block (4138-13653, ~411 KB) prolazi `node --check` izolovano + IIFE-wrap; Playwright pageerror puca bez source location; top-level return scanner (depth/comment/string aware) → 0 nalaza statički. **Likely cause:** dynamic-import / inline-handler / DOMContentLoaded staza emit-uje top-level return u runtime, ne statički u source-u. Regresija nastala posle AL-5.5 ("ALL PERFECT" 11.06 → 10/13 na 15.06). **W47.S1 plan:** chrome `--remote-debugging-port` + verbose source-map stack → fix → re-run eyes 332/332 → kick-off `cortex-slot-builder --closed-loop --rag --pdf <woo>` end-to-end.
