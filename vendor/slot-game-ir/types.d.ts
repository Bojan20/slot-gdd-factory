...
export type SymbolKind = 'lp' | 'hp' | 'wild' | 'scatter' | 'bonus' | 'multiplier' | 'sticky' | 'expanding' | 'mystery' | 'transform' | 'chain_wild';
...
export type BehaviorType = 'expanding_full_reel' | 'walking' | 'transforming' | 'collecting' | 'mystery_reveal' | 'colossal' | 'sticky';
...
export type Feature = {
    ...
    type: 'red_black' | 'suit';
    max_steps: number;
    tie_resolution: 'house' | 'push';
} | {
    kind: 'mystery_symbol';
    symbol_id: SymbolKey;
    reveal_distribution: Record<SymbolKey, number>;
} | {
    kind: 'symbol_upgrade';
    from: SymbolKey;
    to: SymbolKey;
