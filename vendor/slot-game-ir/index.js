 ...
 */
export function parseGameIR(input) {
    ...
    const unknown = collectUnknownTopLevelKeys(input);
 ...
 */
export function crossValidate(ir) {
    const errors = [];
    const warnings = [];
    const symIds = new Set(ir.symbols.map((s) => s.id));
    // ── Symbol uniqueness ──────────────────────────────────────────────
...
// ─── Helpers ───────────────────────────────────────────────────────────
function collectUnknownTopLevelKeys(input) {
    const known = new Set([
        'schema_version',
        'meta',
        'topology',
...
function evalTopologyCoherence(t, e) {
    const issues = [];
    if (e.kind === 'lines') {
        const reels = t.kind === 'rectangular' ? t.reels : t.kind === 'variable_rows' ? t.reels : 0;
...
function paytableShapeCheck(p, e) {
    const issues = [];
