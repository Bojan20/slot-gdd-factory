...
export function parseHitProbabilityRows(input) {
 ...
 */
export function validateMonotonicCoverage(bands, epsilon = 1e-9) {
    if (bands.length === 0) {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (curr.minBet < prev.maxBet - epsilon) {
            throw new Error(`validateMonotonicCoverage: overlap between band [${prev.minBet}, ${prev.maxBet}] and [${curr.minBet}, ${curr.maxBet}]`);
        }
        if (curr.minBet > prev.maxBet + epsilon) {
            throw new Error(`validateMonotonicCoverage: gap between band [${prev.minBet}, ${prev.maxBet}] and [${curr.minBet}, ${curr.maxBet}]`);
        }
    }
    return sorted;
}
/** O(log N) bet → band lookup. Returns null if bet is out of range. */
export function getRtpBandForBet(bands, bet) {
 ...
 * is provided.
 */
export function resolveWinCap(caps, currency, defaultCapX) {
    const direct = caps[currency];
    if (direct !== undefined)
        return direct;
    if (defaultCapX !== undefined)
        return { capX: defaultCapX, mode: 'strict' };
    return null;
}
// ════════════════════════════════════════════════════════════════════════════
 ...
 */
export function getLadderRung(ladder, requestedPaylines) {
 ...
 */
export function checkLadderCompliance(ladder, paylines, bet) {
...
export function jackpotHitProbabilityForBet(tier, bet) {
    for (const b of tier.bands) {
        if (bet >= b.minBet && bet <= b.maxBet)
            return 1 / b.oddsX;
    }
    return 0;
}
 ...
 */
export function pickReelSetVariant(selector, uniform01) {
...
const JsonValueZ = z.lazy(() => z.union([
    z.string(),
 ...
 */
export function getExtra(bag, key) {
